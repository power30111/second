...
m=int(input().rstrip())
array=input().split()	#219진법까지있는듯.  A진수로 들어온 입력을 10진수로 변환하고, 그걸 B진수로 변환해보자
result,j=0,0
for i in range(len(array)-1,-1,-1):
    result+=int(array[i])*(A**j)
    j+=1
#result 는 A가 10진수로 변해있는값이다.
change=deque()
j=0
while result//B != 0 :
    change.appendleft(result%B)
    result=result//B
change.appendleft(result)
print(*list(change))